...
\begin{figure}[h]
	\begin{center}
		\includegraphics[width=0.50\textwidth]{images/nichtinvertierender-versteaerker.png}
	\end{center}
...
Das Osziloskop wurde auf singl Shot, rising Edge gestellt und dann ist an der Steigung von $U_A$ die Slew Rate ablesen.

Aus Abbildung~\ref{fig:slew-rate} werden die Werte $\Delta U = \SI{9.4}{\volt}$ und $\Delta t = \SI{20}{\mu\second}$ abgelesen.
...
Die Slew Rate ist mit $\SI{0.47}{\volt/\mu\second}$ sehr niedrig. Im Skript~\cite[S.~69]{EMTPraktikum2025} steht das typische Werte zwischen $\SI{0.6}{\volt/\mu\second}$ und $\SI{3000}{\volt/\mu\second}$ liegen. Unser OP ist damit als langsamer Verstärker einzuordnen. Im Datenblatt des Herstellers~\cite[S.~150, Abb.~A.3]{EMTPraktikum2025} ist für \SI{25}{\celsius} eine Slew Rate von $\SI{0.5}{\volt/\mu\second}$ typisch. Dieser OP ist also ein wenig langsamer als typisch.
Der Operator muss auf Frequenzen beschränkt werden ($f_{\text{max}}$), bei welcher die Slew Rate mithalten kann. Ansonsten wird das ausgangssignal verzerrt.
\begin{gather}
